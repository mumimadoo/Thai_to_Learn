            ...
            if self.enable_distribution_check:
                ...
                if suspicious_ids:
                    ...
                    if len(batch) > 1 and self.distribution_retry_split_enabled:
                        # Retry logic: split batch
                        mid = len(batch) // 2
                        left_batch = batch[:mid]
                        right_batch = batch[mid:]
                        
                        left_results = self._transcribe_one_batch(audio_path, left_batch, float(left_batch[0]["start"]), float(left_batch[-1]["end"]) - float(left_batch[0]["start"]), preferred_model)
                        right_results = self._transcribe_one_batch(audio_path, right_batch, float(right_batch[0]["start"]), float(right_batch[-1]["end"]) - float(right_batch[0]["start"]), preferred_model)
                        total_gemini_requests += 2
